    ...
    ReentrancyGuard
{
    ...
    function borrowRatePerBlock() external view returns (uint256) {
        (uint256 opaqueErr, uint256 borrowRateMantissa) = interestRateModel
            .getBorrowRate(getCashPrior(), totalBorrows, totalReserves);
        require(
            opaqueErr == 0,
    ...
    function supplyRatePerBlock() external view returns (uint256) {
        ...
        (uint256 e0, uint256 borrowRateMantissa) = interestRateModel
            .getBorrowRate(getCashPrior(), totalBorrows, totalReserves);
    ...
    function accrueInterest() public returns (uint256) {
        ...
        (vars.opaqueErr, vars.borrowRateMantissa) = interestRateModel
            .getBorrowRate(getCashPrior(), totalBorrows, totalReserves);
        require(
    ...
    function liquidateBorrowFresh(
        ...
        uint256 repayBorrowError = repayBorrowFresh(
            liquidator,
            borrower,
            repayAmount
        );
        ...
        uint256 seizeError = cTokenCollateral.seize(
            liquidator,
            borrower,
            seizeTokens
        );
...
contract CEther is CToken {
    ...
    //@audit 因为CEther继承了CToken, 但是CToken合约需要一些创建参数，可以通过这个方法，在CEther创建的时候，传给CToken合约参数。
    //参考文档：https://docs.soliditylang.org/en/v0.8.13/contracts.html#constructor
    constructor(
        ComptrollerInterface comptroller_,
        InterestRateModel interestRateModel_,
        uint256 initialExchangeRateMantissa_,
